...
| `backend/app/tests/unit/test_dlp_enforcement.py` | 13 | Inline DLP blocking (`dlp_enforcement.py`, 2.9.44): the pure `_active` gate (master-off, no block-severities, prompt/response/both scope filtering), `DlpBlockedError.client_message` names categories but never the matched value, and `evaluate_prompt_block`/`evaluate_response_block` — returns None when blocking off (no scan), blocks when a finding maps to a block-configured severity (correct severity + categories), no-block when the finding's severity isn't set to block, **fail-open** on scanner exception AND on a degraded empty result, empty text never blocks, response-scope leaves the prompt path inactive. Deps (crud/scanner/worker) faked via `patch.dict(sys.modules)` context managers to dodge the import-chain + pollution gotchas |
