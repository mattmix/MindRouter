...
| `backend/app/tests/unit/test_moderations_endpoint.py` | 12 | Standalone `POST /v1/moderations` (commit d60c224): judges a text against `img.policy` with the same fail-closed LLM judge as image generation, without generating — client apps vet vision-model captions of reference images before img2img. Covers: pass/fail verdict round-trip, edit-context template selection, no-policy short-circuit (passes AND is deliberately unmetered — no LLM call happens), 400s for missing/blank/oversized input and bad context, and the METERING contract added post-review: quota+RPM checked BEFORE the judge runs (429 prevents the GPU call), every judged call recorded as a CHAT-modality Request row attributed to the judge model (voice/video precedent for endpoints dispatching GPU work outside InferenceService — without it any inference-scoped key could drive the judges in an invisible unthrottled loop), and a comment-stripped route-decorator guard (the behavioral tests call the function directly, so a deleted `@router.post` would otherwise vanish the endpoint with tests green) |
