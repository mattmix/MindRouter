...
| `backend/app/tests/unit/test_blog_website_publish.py` | 9 | Blog syndication contract (source-inspection): migration 064 columns, model fields, CRUD selection query (published+undeleted only), flag routes guarded + flag-only (no push), pull-model guards — website_publisher.py stays deleted, no push credentials in settings, /blog/feed.xml + /api/blog/syndicated exist, are public, and read the selection query |
