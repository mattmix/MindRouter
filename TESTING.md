...
| `backend/app/tests/unit/test_video_worker_client.py` | 4 | HttpVideoWorkerClient auth + TLS wiring (importlib-loaded to skip the db import chain, fake httpx.AsyncClient captures init/request kwargs): submit sends `X-Worker-Key` when `api_key` set and threads `verify_tls` into `AsyncClient(verify=...)`; no key → no header + `verify=False`; cancel sends the key; `_headers()` helper (2.9.21 video-worker auth) |
