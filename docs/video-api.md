...
| `seed` | int | no | random | Determinism seed. When omitted, a random seed is generated **at submit time, persisted, and echoed** on the job object — so every render is reproducible after the fact, and repeat submissions of the same prompt produce different takes. **Scope of determinism** (verified empirically, 2026-07-26): a fixed seed reproduces the **video track bit-exactly**; the model's native **audio track re-rolls on every render** regardless of seed, so whole-file checksums differ. If you need stable audio, extract and reuse the track from a chosen take (or replace it, e.g. with TTS narration). |
| `negative_prompt` | string | no | — | **Ignored.** The distilled model is guidance-free (CFG=1), so negative prompts cannot take effect; sending one gets a logged hint (or a `400` in enforce mode). Steer content through the positive prompt instead — the model honors prose direction like "no speech, only wind and distant machinery". |
